...
const cloneEssayValues = (value: AdminEssayEditorValues): AdminEssayEditorValues => ({
  ...
  authorAvatarsText: value.authorAvatarsText,
  authorShowAvatar: value.authorShowAvatar,
  translationTranslator: value.translationTranslator,
  translationAvatar: value.translationAvatar,
  translationShowAvatar: value.translationShowAvatar,
  translationSource: value.translationSource,
  translationSourceUrl: value.translationSourceUrl
});
...
const clonePicksValues = (value: AdminPicksEditorValues): AdminPicksEditorValues => ({
  title: value.title,
  date: value.date,
  year: value.year,
  status: value.status,
...
const cloneMaterialsValues = (value: AdminMaterialsEditorValues): AdminMaterialsEditorValues => ({
  title: value.title,
  href: value.href,
  date: value.date,
  label: value.label,
...
export const isPicksEditorValues = (value: AdminContentWorkspaceEditorValues | null): value is AdminPicksEditorValues =>
  Boolean(value && 'authorsText' in value && 'tagsText' in value && 'status' in value && 'year' in value);

export const isMaterialsEditorValues = (value: AdminContentWorkspaceEditorValues | null): value is AdminMaterialsEditorValues =>
  Boolean(value && 'href' in value && 'label' in value && 'description' in value && !('draft' in value));
...
const PICKS_FRONTMATTER_ISSUE_PATHS = new Set([
  'title',
  'date',
  'year',
  'status',
...
const MATERIALS_FRONTMATTER_ISSUE_PATHS = new Set([
  'title',
  'href',
  'date',
  'label',
...
const ESSAY_FIELD_LABELS: Readonly<Record<string, string>> = {
  ...
  authorName: '作者',
  authorAvatar: '作者头像',
  authorAvatarsText: '作者头像',
  author: '作者',
  authors: '作者',
  translationTranslator: '译者',
  translationAvatar: '译者头像',
  translationSource: '原文来源',
  translationSourceUrl: '原文链接',
  translation: '译者信息',
  body: '正文'
};
...
const PICKS_FIELD_LABELS: Readonly<Record<string, string>> = {
  title: '标题',
  date: '日期',
  year: '年份',
  status: '状态',
...
const MATERIALS_FIELD_LABELS: Readonly<Record<string, string>> = {
  title: '标题',
  href: '链接',
  date: '日期',
  label: '标签',
...
const ESSAY_ADAPTER: ContentEditorAdapter<AdminEssayEditorValues> = {
  collection: 'longform',
  capabilities: buildContentEditorCapabilities('longform', {
    body: true,
    preview: true,
...
const PICKS_ADAPTER: ContentEditorAdapter<AdminPicksEditorValues> = {
  collection: 'picks',
  capabilities: buildContentEditorCapabilities('picks', {
    body: true,
    preview: true,
    bodyImageInsert: false,
    bodyGalleryInsert: false,
  ...
  frontmatterIssuePaths: PICKS_FRONTMATTER_ISSUE_PATHS,
  isFrontmatterIssuePath: (path) => hasExactFrontmatterIssuePath(PICKS_FRONTMATTER_ISSUE_PATHS, path),
  cloneValues: clonePicksValues,
  isEqualValues: isEqualContentEditorValues,
  getWriteFieldLabel: (field) => getContentWriteFieldLabel(field, PICKS_FIELD_LABELS),
  getDeleteTitle: (value, entryId) => value.title || entryId
};

const MATERIALS_ADAPTER: ContentEditorAdapter<AdminMaterialsEditorValues> = {
  collection: 'materials',
  capabilities: buildContentEditorCapabilities('materials', {
    body: false,
    preview: false,
    bodyImageInsert: false,
    bodyGalleryInsert: false,
  ...
  frontmatterIssuePaths: MATERIALS_FRONTMATTER_ISSUE_PATHS,
  isFrontmatterIssuePath: (path) => hasExactFrontmatterIssuePath(MATERIALS_FRONTMATTER_ISSUE_PATHS, path),
  cloneValues: cloneMaterialsValues,
  isEqualValues: isEqualContentEditorValues,
  getWriteFieldLabel: (field) => getContentWriteFieldLabel(field, MATERIALS_FIELD_LABELS),
  getDeleteTitle: (value, entryId) => value.title || entryId
};
...
type ContentEditorAdapterMap = {
  longform: ContentEditorAdapter<AdminEssayEditorValues>;
  bits: ContentEditorAdapter<AdminBitsEditorValues>;
  picks: ContentEditorAdapter<AdminPicksEditorValues>;
  materials: ContentEditorAdapter<AdminMaterialsEditorValues>;
...
const CONTENT_EDITOR_ADAPTERS = {
  longform: ESSAY_ADAPTER,
  bits: BITS_ADAPTER,
  picks: PICKS_ADAPTER,
  materials: MATERIALS_ADAPTER,
  memo: MEMO_ADAPTER,
  about: ABOUT_ADAPTER
