...
export type AdminContentDeleteConfirmationPayload = Pick<AdminContentSourceState, 'collection' | 'entryId' | 'publicEntryId' | 'defaultPublicSlug' | 'revision' | 'relativePath'> & {
  writable: false;
...
export class AdminContentDeleteConfirmationError extends Error {
  ...
  readonly payload: AdminContentDeleteConfirmationPayload;

  constructor(
    code: 'revision-conflict' | 'relative-path-mismatch',
    payload: AdminContentDeleteConfirmationPayload
  ) {
...
const buildDeleteConfirmationPayload = (state: AdminContentSourceState): AdminContentDeleteConfirmationPayload => ({
  ...
  writable: false,
  readonlyReason: getAdminContentCollectionCapability(state.collection).readonlyReason
});
...
export const deleteAdminContentEntryWithConfirmation = async (
  ...
  const currentState = await loadAdminContentSourceState(collection, entryId);
  const currentPayload = buildDeleteConfirmationPayload(currentState);
