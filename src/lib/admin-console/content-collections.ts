export type AdminContentCollectionKey = 'longform' | 'bits' | 'picks' | 'materials' | 'memo' | 'about';
...
export const ADMIN_CONTENT_COLLECTION_KEYS = ['longform', 'bits', 'picks', 'materials', 'memo', 'about'] as const satisfies readonly AdminContentCollectionKey[];
...
export const ADMIN_CONTENT_COLLECTION_CAPABILITIES = {
  ...
  memo: {
    ...
    imageUpload: true,
    imagePicker: false,
    readonlyReason: '本项目未启用 memo 集合；此项仅保留上游编辑器兼容。',
    deleteUnsupportedReason: '本项目未启用 memo 集合。'
  },
  about: {
    collection: 'about',
    label: '关于（隐藏兼容）',
