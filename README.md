...
- 长文：位于 `src/content/longform` 目录；后台新建条目会写入 `src/content/longform/YYYYMM/`
- 絮语：位于 `src/content/bits` 目录；后台与 `npm run new:bit` 新建条目会写入 `src/content/bits/YYYYMM/`
- 拾选：条目位于 `src/content/picks/YYYYMM/YYYY-MM-DD-HHMMSS.md`，`src/content/picks/index.md` 仅保留页面标题、介绍等元数据
- 归档：由长文集合按 `archive` 字段生成目录视图
- 资料：位于 `src/content/materials`，后台新建条目会写入 `src/content/materials/YYYYMM/`，用于维护视频或文件链接
...
author:             # 可选；未填时沿用 /bits/ 的默认作者与头像
  name: Alice
  avatar: author/alice.webp
  showAvatar: true  # 设为 false 可隐藏头像，仅显示作者名
authors:            # 可选；多作者时优先使用 authors 数组
  - name: Alice
    avatar: author/alice.webp
  - name: Bob
    showAvatar: false
translation:        # 可选；适合翻译文章
  translator: WZVico
...
tags:              # 可选；待拾也可填写标签
  - 科幻
# status: planned  # 可选；待拾条目不需要正文或备注
```

`status` 默认为 `shared`，会按年份进入正式拾选分享；`status: planned` 会显示在 `/picks/` 顶部的“待拾”分组。若同名同作者已经存在正式拾选分享，对应待拾条目会自动隐藏。
...
长文（longform）详情页也会显示作者信息，位置在标题与发布日期/标签/字数元信息之间：

- 默认作者与头像沿用 Theme Console 的 `page.bits.defaultAuthor`，本仓库默认作者为 `WZVico`
- 单篇长文可在 frontmatter 用 `author.name` / `author.avatar` 覆盖；`author.showAvatar: false` 可隐藏头像
- 多作者长文可用 `authors` 数组；详情页会按「作者 → 头像 → 名称」逐个显示，`authors` 优先于单个 `author`
- 翻译文章可填写 `translation.translator`、`translation.avatar`、`translation.source`、`translation.sourceUrl`；译者显示在作者后面，文章来源显示在元信息下方并链接到原文
- 长文详情页只显示已配置的头像；多作者或译者没有单独头像时只显示名称，不会套用默认作者头像

- `/bits/` 头像图片缺失或加载失败时，会自动回退到首字母头像
