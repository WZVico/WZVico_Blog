...
const forbiddenResolvedModules = new Map([
  [normalizeComparablePath(path.join(projectRoot, 'src/lib/content.ts')), 'public longform/picks content module import'],
  [normalizeComparablePath(path.join(projectRoot, 'src/lib/bits.ts')), 'public bits module import'],
  [normalizeComparablePath(path.join(projectRoot, 'src/lib/materials.ts')), 'public materials module import'],
  [normalizeComparablePath(path.join(projectRoot, 'src/lib/home-index.ts')), 'public home index module import']
]);

const forbiddenSourcePatterns = [
  {
    label: 'astro content entry type',
    pattern: /\b(?:CollectionEntry|LongformEntry|BitsEntry|ReadsEntry|MaterialsEntry)\b/gu
  },
  {
    label: 'public content query',
    pattern: /\b(?:getPublished|getSortedLongforms|getVisibleLongforms|getArchiveLongforms|getVisibleLongformRouteEntries|getLongformDerivedText|getReadsDerivedText|getLongformSlug|getSortedBits|getBitsPageData|getBitsSearchIndex|getBitsDerivedText|getBitSlug|getMaterials|groupMaterials)\b/gu
  }
];
